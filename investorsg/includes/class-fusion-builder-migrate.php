...
class Fusion_Builder_Migrate {
	 ...
	 * Please note this does not refer to a post-ID, but the query's offset.
	 * This is calculated by adding posts_per_page() to $from
	...
	public function init() {
		...
		if ( 'query' === $_GET['step'] ) {
			...
			self::$widgets_to_convert = $this->posts_per_page();
	...
	private function next_step_redirection( $advance = false ) {
		...
		if ( 'query' === $_GET['step'] ) {
			...
			if ( ! $query_finished ) {
				...
				$url = admin_url( 'index.php?' . self::$slug . '=1&step=convert&type=posts&size=50' . $revert );
			}
		} elseif ( 'convert' === $_GET['step'] ) {
			$revert = ( self::$revert ) ? '&revert=1' : '';
			if ( 'posts' === $_GET['type'] ) {
	...
	/**
	 * Calculates the posts per page for each step.
	 *
	 * @access private
	 * @since 5.0.0
	 * @return int
	...
	private function posts_per_page() {
		...
		if ( function_exists( 'ini_get' ) ) {
			...
			$posts_per_page = round( $max_execution_time / 2 );
			return min( 100, max( 10, $posts_per_page ) );
		}
		return 15;
	...
	private function get_migration_page_buttons() {
		...
		<?php if ( ! isset( $_GET['step'] ) ) : ?>
			...
			<?php $posts_per_page = $this->posts_per_page(); ?>
